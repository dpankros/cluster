...
MongoDiscovery.connect = function connect(mongoUrl, clusterInstance, options) {
  ...
  this._changeCallbacks = [options.changeCallbacks] || this._changeCallbacks || [];
  if ( changeCallbacks && this.changeCallback) {
    this._changeCallbacks.push(this.changeCallback);
  }
};
    ...
    if (callbacks && callbacks.length > 0) { //do we have any callbacks?
      if (removed.length > 0 || added.length > 0) {
        ...
        callbacks.forEach(function(cb){
          ...
          if (cb) cb.call({}, {store: options.store, removed: removed, added: added});
        });
      }
    }
