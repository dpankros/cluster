...
function _observerAndStore(cursor, store, options) {
  ...
  function fecthAndWatch() {
    ...
    var allServices = cursor.fetch().filter(MongoDiscovery._isHealthy);

    var previousServiceIds = _.keys(existingServices);
    var newServiceIds = allServices.map(function(service) {
      return service._id.toString();
    });
    var removed = [];
    var added = [];
    var existing = [];
    var removedServiceIds = _.difference(previousServiceIds, newServiceIds);
    var addedServiceIds = _.difference(newServiceIds, previousServiceIds);
    var existingServiceIds = _.intersection(previousServiceIds, newServiceIds);

    removedServiceIds.forEach(function(id) {
    ...
    added = addedServiceIds.map(function(id){return store.get(id)});
    existing = existingServiceIds.map(function(id){return store.get(id)});
...
MongoDiscovery.allEndpoints = function allEndpoints() {
  return this._currentEndpoints;
};

MongoDiscovery.allBalancers = function allBalancers() {
  return this._currentBalancers;
};